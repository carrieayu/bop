...
export const fetchBusinessDivisions = createAsyncThunk('business/fetch', async () => {
  try {
    const response = await api.get<BusinessDivisionsEntity[]>('http://127.0.0.1:8000/api/master-business-divisions/list/')
    // const response = await api.get<BusinessDivisionsEntity[]>('http://54.178.202.58:8000/api/master-business-divisions/list/')
